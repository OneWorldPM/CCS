...
<script>
    ...
    var cp_name = "<?= $c_name ?>";
    var session_start_datetime = "<?= date('M d, Y', strtotime($sessions->sessions_date)) . ' ' . $sessions->time_slot . (isset($timezone) && !empty($timezone))?$timezone:'UTC-4' ?>";
    var session_end_datetime = "<?=date('M d, Y', strtotime($sessions->sessions_date)) . ' ' . $sessions->end_time . (isset($timezone) && !empty($timezone))?$timezone:'UTC-4' ?>";
    //Conflict resolver
...
<?=pubnub_keys()?>
<script>let pubnub_channel = "CCS_Session_<?=$sessions->sessions_id?>";</script>
