...
//        }
    ...
    public function hide_question() {
        ...
        if ($post['sessions_question_id'] != '') {
            $this->db->insert('cust_question_hide_status', array('hidden_from' => 'admin', 'hidden_from_id'=>$this->session->userdata['aid'],'sessions_cust_question_id' => $post['sessions_question_id']));
            if ($this->db->affected_rows()) {
                $result_array = array("status" => "success");
            } else {
                $result_array = array("status" => "error");
