...
//        }
    ...
    public function poll_chart($session_id)
    {
        ...
        foreach ($poll_data as $poll)
        {
            ...
            $pdf->SetXY(10, 30);
            $pdf->WriteHTML($poll->poll_name.': '.$poll->question, '', 0, '', true, 'C', false, false, 0);
            ...
            foreach ($poll->options as $option)
            {
                if ($poll->total_votes != 0)
                {
                    ...
                    if ($percent != 0)
                    {
                        ...
                        $pdf->WriteHTML( $option->option, '', 0, '', true, 0, false, false, 0);
            ...
            foreach ($poll->options as $option)
            {
                ...
                {
                    $result_table .= '<tr>
                                    <td style="width: 500px; height: 10px;">'.($option->option).'</td>
    ...
    {
        $this->db->select('CONCAT(customer_master.first_name, " ", customer_master.last_name) AS user_name, ask_a_rep.*');
        $this->db->from('ask_a_rep');
        $this->db->join('customer_master', 'customer_master.cust_id = ask_a_rep.user_id');
